...
class ExtraWatchHeatmapHTML
{
  ...
  function __construct($database)
  {
    ...
    function renderHeatmapTableRow($row, $day, $maxClickForDay, $i)
    {
        ...
        return sprintf("<tr class='tableRow" . ($i % 2) . "'><td align='center' style='color: %s'>%d</td><td>%s</td><td>%s</td>%s</tr>", $color, $row->count, $this->renderHeatmapLink($uri, $row->uri2titleId, $day, $title, "", TRUE), ExtraWatchHelper::truncate($uri, 30), $trendsCells);
    }
    ...
    function renderHeatmapLink($uri, $uri2TitleId, $day, $linkContent, $ip = "", $truncate = FALSE)
    {
        ...
        return sprintf("<a href='%s". $separator . ExtraWatchHeatmap::HEATMAP_PARAM_NAME . "=%d&" . ExtraWatchHeatmap::HEATMAP_PARAM_DAY_NAME . "=%d&ip=%s&" . ExtraWatchHeatmap::HEATMAP_PARAM_HASH . "=%s&amp;uri2titleId=$uri2TitleId' target='_heatmap'>%s</a>", $uri, 1, $day, $ip, $this->extraWatch->database->getEscaped($this->extraWatch->config->getRandHash()), $linkContent);
    }
    ...
    function renderMostClickedHTMLElementsTable($day) {
        ...
        $output .= sprintf("<tr><th>%s</th><th>%s</th><th>%s</th><th>%s</th><th>%s</th><th>%s</th></tr>", _EW_HEATMAP_CLICKS, _EW_HEATMAP_TITLE, "xpath", _EW_EMAIL_REPORTS_1DAY_CHANGE, _EW_EMAIL_REPORTS_7DAY_CHANGE, _EW_EMAIL_REPORTS_28DAY_CHANGE);
        $i = 0;
        ...
        if (@$rows) {
            ...
            foreach($rows as $row) {
                ...
                $output .= "<tr  class='tableRow" . ($i % 2) . "'>".
                    ...
                    "<td><a href='".$row->uri."' title='".htmlspecialchars($row->title)."' target='_blank'>".ExtraWatchHelper::truncate($row->title)."</a></td>".
                    "<td title='".htmlspecialchars($row->xpath)."'>".ExtraWatchHelper::truncate($row->xpath)."</td>".
                    $trendsCells.
