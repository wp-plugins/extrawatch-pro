...
<table align="left" border='0' style='border: 1px solid #dddddd; padding: 2px;' border-collapse="collapse" width="100%">
  ...
  foreach ($rows0 as $row) {
    ...
    */
    ...
    $trendsIcon = "<img src='" . $extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/img/icons/trend_icon.gif' border='0'  " . $extraWatch->helper->getTooltipOnEvent() . "=\"ajax_showTooltip('" . $extraWatch->config->getLiveSiteWithSuffix() . "<?php echo $extraWatch->env->renderAjaxLink('ajax','trendtooltip.php?rand=" . $extraWatch->config->getRand() . "&group=" . EW_DB_KEY_SIZE_DB . "&name=" . $row->Name . "&date=$day&env=".get_class($extraWatch->env)."&username="._EW_USERNAME."',this);return FALSE\"/>";
